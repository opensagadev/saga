...
At the latest measured target build, there are 508 current translation
units and a 45.9249% whole-binary fuzzy match. The independent structural
dashboard reports 46.7% largest-run text grouping and 92.3% of assessable
same-TU writable-state links satisfied; neither is a completion percentage.
...
it is recomputed. To compare a proposed TU slice,
run:
...
| `game_deb.cpp` action/context lookup data: adjacent `ExtraActionData`, exported pointers, and paired local tables before its initializer | `legoapi/render/fx/game_deb.cpp` and canonical `game_deb.h`; previously `globals.cpp` | Complete data cluster moved together. Target object retains original names, binding and sizes (`0x108`, `0x750`, `0x650`); function scores are unchanged. **Open:** original linked order is `ExtraActionData`, `ActionInfo`, `CInfo`, `ActionInfoList`, `_CInfoTab`, while current object emits the pointers before `ExtraActionData`. All five share `.data.rel.local`; avoid splitting the evidenced TU or changing visibility just to force layout. A no-top-level-reorder flag is only a hypothesis and needs a whole-unit codegen test before use. |
...
| Hub select/bonus menus: consecutive `MenuInit`/`Update`/`Draw` callbacks in original `0x1b50c0–0x1b6440` menu run, accessing hub-local state | `legoapi/world/levels/hub.cpp`; formerly four callbacks and their shared free-play predicate in `gamemenuall.cpp` | The complete select/bonus lifecycle now shares the hub state owner, closing six verified splits without changing body matching. Public callback declarations are in `gameapi/gui/apimenu.h`, not local link-time prototypes. Larger menu-run ownership and optimization still need review. |
...
| Bolt-type parser: local `BT_*` helpers/state in the original `0x47ee30–0x480d60` bolt run | `gameapi/ai/gameai_bt.cpp`, `legoapi/items/collect/bolts.cpp` | The existing `BoltTypes_Configure` stub now shares the four local parser-state objects it references in the original, closing four verified splits with unchanged body matching. **Open:** the complete bolt TU boundary and source filename; two distinct bolt-named initializers preclude a basename-only merge. |
| Cutscene instance lists: `instNuGCutSceneFind`/`CleanUp` use local active/background lists | `legoapi/cutscenes/cutscene.cpp`; formerly the two stubs in `cutscene_stubs.cpp` | Both stubs now share the evidenced list owner, closing three verified splits with unchanged body matching. The rest of the broad cutscene stub file is not yet assigned wholesale. |
...
| Terrain data/collision: `gameliball.cpp` initializer block owns local `TerI`, `SphereData`, `PlatCallback`, and impact data | `legoapi/render/core/terrain*.cpp`, `gameliball.cpp`, terrain functions in `hits.cpp`, `transform.cpp`, `surfaces.cpp`, `episode.cpp`, `androidbatman.cpp` | **Open.** `TerI` users span several files, while the `NewScanHandel*` stubs in `androidbatman.cpp` reference `TempStackPtr`/`WallSplCount` in `terrain.cpp` and other scan state remains in `terrain_stubs.cpp`. The original `0x36de60–0x38f460` run interleaves terrain and animation. Consolidate the dependent scan/collision state and helpers as a complete cut before moving those stubs; a state-only move would need a false cross-TU bridge. |
| Editor tools: local camera/UI/cursor/menu state under `_GLOBAL__sub_I_edtoolsall.cpp`, text span `0x330900–0x3a9c88`; RTL follows | `gameapi/edtools/edtoolsall_plain.cpp`, `edtoolsall.cpp`, `edptlall.cpp`, remaining editor callbacks | **Open.** The `-O2` plain owner now also holds the 49 local editor-UI callbacks from the removed `edui.c`, plus two menu callbacks referencing its configuration/module state: 42 verified splits closed with body matching preserved. Two repeat-box callbacks now share the particle editor owner with both referenced sliders, closing four more splits. Six called item-creation stubs now expose typed pointer-returning declarations in `edui.h` instead of mismatched local prototypes. Complete source order, remaining callback/state groups, and original optimization need further evidence. |
