...
| `nucamera.c`: original run `[0x2a3b0c, 0x2a5386)` ending in `_GLOBAL__sub_I_nucamera.c` | `nu2api/nu3d/nucamera.cpp` plus an exact matrix helper in `nuvec.cpp` | **Next owner review.** Five of ten run symbols share the current camera object. Original `clip_test_mtx`, six local vectors, and callback-local `lastId` support the boundary; callback order, the matrix helper, and `.c` basename differ. Validate optimization before moving an exact body. |
