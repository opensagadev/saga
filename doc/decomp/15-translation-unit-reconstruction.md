...
| Terrain data/collision: `_GLOBAL__sub_I_gameliball.cpp` and original strong-local component 463 | `legoapi/render/core/terrain*.cpp`, `characters/motion/charplatforms.cpp` | **Open:** its 91 functions/48 LOCAL objects now map to three current sources (43 terrain stubs, 47 terrain core, one character), down from ten at survey time. All 34 unpaired LOCAL objects have one same-sized GLOBAL counterpart (11 core, 23 stubs); co-locate users before restoring local binding. Terrain APIs have a real owner header, and the wall-deflect, rotation, scan-control, pickup, extended-shadow, and seven-function collision groups are more closely placed without an exact-match loss. This is a minimum same-TU constraint, not proof that adjacent editor code belongs here. |
