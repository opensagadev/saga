...
At the latest measured target build, there are 501 current translation
units and a 45.9957% whole-binary fuzzy match. The independent structural
dashboard reports 47.9% largest-run text grouping and 98.8% of assessable
...
| Editor path connection state: one original LOCAL `aipathcnxtypes`/`naipathcnxtypes` pair is referenced by path UI, registration and clear functions | `editor/aieditor.cpp` and `editor/edpath.cpp` | **Open:** both current files define incompatible duplicate 0x4c-byte connection types and separate state. A single typed state owner is required, but the long interleaved run does not prove a full TU cut; protect the 29 already-exact bodies across both files before consolidation. |
