...
At the latest measured target build, there are 500 current translation
units and a 45.9959% whole-binary fuzzy match. The independent structural
dashboard reports 48.0% largest-run text grouping and 99.1% of assessable
...
| Terrain data/collision: `gameliball.cpp` initializer block owns local `TerI`, `SphereData`, `PlatCallback`, and impact data | `legoapi/render/core/terrain*.cpp`, `gameliball.cpp`, terrain functions in `hits.cpp`, `transform.cpp`, `surfaces.cpp`, `episode.cpp`, `androidbatman.cpp` | **Open.** The original local `TerI` has verified references from 54 scan/collision/raycast/shadow/platform functions now spread across files. The `NewScanHandel*` stubs also reference `TempStackPtr`/`WallSplCount` in `terrain.cpp`. The original `0x36de60–0x38f460` run interleaves terrain and animation. Consolidate the `TerI`-dependent group as a complete cut; moving only stubs or state would retain a false cross-TU bridge. |
