...
4. GCC 4.7 retains the source operand order for some floating compares. In `Boulder_Move`, spelling the threshold as `1.0f < part->scale_time` emits target `ucomiss constant, field; jae` and the target's 463-byte body. Spelling the equivalent test as `part->scale_time > 1.0f` emitted `ucomiss field, constant; jbe` and a 466-byte body in a direct NDK r8e compile.
