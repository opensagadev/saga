...
MechInputTouchDeathStarTurretController::MechInputTouchDeathStarTurretController(i32 player_index)
    : MechInputTouchMainController(player_index), active(0), aim_touch(NULL), turret(NULL) {
}
...
void MechInputTouchDeathStarTurretController::Update(NuInputTouchData const *) {
    ...
    if (aim_touch != NULL && player != NULL && turret != NULL) {
        ...
        const i32 camera_angle = NuAtan2D(GameCam->pos.z - GameCam->target.z,
                                          GameCam->pos.x - GameCam->target.x);
        const u16 yaw_target = static_cast<u16>(horizontal_angle - (camera_angle - 0x8000));
