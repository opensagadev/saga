...
static i32 Action_CanCollideWithObjects(AISYS *, AISCRIPTPROCESS *, AIPACKET *packet, char **params,
                                      i32 param_count, i32 first_time, f32) {
    if (packet == NULL || packet->owner == NULL)
        return 1;
    GameObject_s *object = packet->owner->apiobj.objptr;
...
static i32 Action_CanMoveWhenDeactivated(AISYS *, AISCRIPTPROCESS *, AIPACKET *packet, char **params,
                                       i32 param_count, i32, f32) {
    if (packet == NULL || packet->owner == NULL)
        return 1;
    GameObject_s *object = packet->owner->apiobj.objptr;
...
static i32 Action_CanTriggerObstacle(AISYS_s *, AISCRIPTPROCESS_s *, AIPACKET_s *, char **params,
                                     i32 param_count, i32 first_time, f32) {
    if (first_time == 0 || param_count == 0) {
        return 1;
    }
...
static i32 Action_PathConnectionObstacle(AISYS_s *sys, AISCRIPTPROCESS_s *, AIPACKET_s *, char **params,
    ...
    if (sys != NULL && first_time != 0) {
        ...
        if (to != NULL && from != NULL) {
            AIPATHCNX *connection =
                static_cast<AIPATHCNX *>(AIPAthFindPathCnx(sys, NULL, from, to, &direction));
...
static f32 Condition_NearestPlayerRange(AISYS_s *sys, AISCRIPTPROCESS_s *processor, AIPACKET_s *packet, char *, void *void_arg) {
    NUVEC difference;
...
static f32 Condition_NearestPlayerXZRange(AISYS_s *sys, AISCRIPTPROCESS_s *processor, AIPACKET_s *packet, char *, void *void_arg) {
    NUVEC difference;
    f32 distance = 3.402823466e+38f;
    if (packet != NULL && packet->owner != NULL && sys != NULL) {
        if (sys->player_1 != NULL) {
...
i32 Action_CircleOpponent(AISYS *, AISCRIPTPROCESS *processor, AIPACKET *packet, char **params,
    ...
    if (packet->opponent_object != NULL && packet->opponent_object->ai != NULL) {
        ...
        AIMoveInstruction(packet, position, target->mover_height, &target->path_info,
...
i32 Action_CirclePlayer(AISYS *sys, AISCRIPTPROCESS *processor, AIPACKET *packet, char **params,
                       i32 param_count, i32 first_time, f32) {
    NUVEC difference;
    if (packet == NULL || sys == NULL || sys->player_1 == NULL) {
        return 1;
