#include "decomp.h"
#include "gameapi_edtools_types.h"

// Graph editor subsystem stubs (static, internal linkage).

static void edgraClose() {
    STUBBED();
}
static void edgraEnter() {
    STUBBED();
}
static void edgraRender() {
    STUBBED();
}
static void edgracbFileLoad(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbFileSave(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbCopyClump(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgraToggleFilter(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSScaleMenu(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbDiscardCopy(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbGlobalsMenu(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSetDpadMode(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbChangeSScale(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbDpadModeMenu(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbInstanceMenu(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSetClumpArea(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSetClumpDist(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSetClumpMode(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSetClumpWind(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbCancelOptMenu(eduimenu_s *, eduimenu_s *) {
    STUBBED();
}
static void edgracbClumpAreaMenu(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbClumpDistMenu(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbClumpFadeMenu(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbClumpModeMenu(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbClumpSizesMenu(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSetClumpFadeIn(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSetClumpHeight(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbApplyGlobalFade(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSetClumpFadeOut(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSetGlobalFadeIn(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbSetInstanceType(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgracbToggleClumpTilt(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
static void edgraChangeFilterName(eduimenu_s *, eduiitem_s *, u32) {
    STUBBED();
}
