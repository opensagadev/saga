...
GameObject_s *ObjOpponent(GameObject_s *object, f32 range, f32 extra_radius,
                          i32 allow_untargeted, i32 mode, i32 player_filter) {
    ...
    for (i32 i = 0; i < HIGHGAMEOBJECT; ++i) {
        ...
        if ((object->apiobj.flags_low & 0x80) != 0) {
            if (nextShootTarget != NULL && nextShootTarget->GetCharacterObject() != target) continue;
        } else if (test_ai_combo != 0 && object->ai.action_target_ref != NULL && *object->ai.action_target_ref != NULL) {
            target = *object->ai.action_target_ref;
            i = HIGHGAMEOBJECT;
        }
        if (target == object || (target->apiobj.field_0x1f8 & 0x1001) != 0x1001 ||
            target->apiobj.field_0x287 != 0 || target->apiobj.collision_min.y > object->apiobj.collision_max.y) continue;
        if ((player_filter == 1 && target->apiobj.field_0x27c != -1) ||
            (player_filter == 2 && target->apiobj.field_0x27c == -1)) continue;
        ...
        if ((context == 0x17 && (target->current_hp == 0 || (runtime->flags_090 & 0x40) != 0)) ||
            (CInfo[context].flags & 0x8000) != 0 || (context & 0xfd) == 0x39 || context == 0x3c) continue;
        if (context == 0) {
            if (static_cast<u8>(target->action_movement_state - 3) < 2) continue;
        } else if (static_cast<u8>(context - 0xd) < 2 || context == 0x35 || context == 0x47 || context == 0x46) continue;
        if ((runtime->flags_090 & 0x8000) != 0 || target == object->field_0xcc0) continue;
        u32 object_flags = object->apiobj.field_0x1f4;
        u32 target_flags = target->apiobj.field_0x1f4;
        if ((object_flags & 0x10000) == 0 && (target_flags & 0x10000) == 0 &&
            (object->apiobj.flags_low & 0x80) == 0 && ((object_flags ^ target_flags) & 5) == 0) continue;
        if (mode != 0) {
            if (static_cast<u8>(context + 0xa1) < 2 ||
                (context == 0x5a && (target->field_0x7a3 == 0 || (object->apiobj.flags_low & 0x80) == 0)) ||
                (context == 0x17 && target->hitpoints == 0)) continue;
            if (object->apiobj.field_0x27c != -1) {
                if ((object->apiobj.flags_low & 0x80) != 0) {
                    if (target->apiobj.field_0x27c == -1) {
                        if (WORLD->current_level != HUB_LDATA && (target_flags & 0x10005) == 0) continue;
                    } else {
                        if (((object_flags ^ target_flags) & 1) == 0 &&
                            (object->ai.action_target_ref != NULL || target->ai.action_target_ref != NULL)) continue;
                        if (CanPunchGirls == 0 && (object->field_0xf01 & 8) == 0 &&
                            (target->field_0xf01 & 8) != 0 && object->id != id_GAMORREANGUARD) {
                            i32 move = SpecialMove_Check(object, target);
                            if (move == -1 || SpecialMove_GetFlags(move, 1) == 0) continue;
                        }
                    }
                } else if (target->apiobj.field_0x27c != -1 ||
                           (WORLD->current_level != HUB_LDATA && (target_flags & 0x10005) == 0)) continue;
            }
            if ((target->apiobj.character_data->model_flags & 0x2000) != 0 ||
                ((runtime->flags_090 & 0x40) != 0 && (target->apiobj.flags_low & 0x80) == 0)) continue;
        }
        if (!(allow_untargeted != 0 && (ignore_ai != 0 || object->ai.action_target_ref == NULL || mode != 0)) &&
            (target_flags & 1) == 0) continue;
        NUVEC delta;
        ...
        if (forward.x * delta.x + forward.z * delta.z < 0.0f) {
            ...
            if (object->combo_stage == 0) {
                if (object->apiobj.character_model->model_data_b[0x94] == NULL) continue;
                behind = 1;
            }
        }
