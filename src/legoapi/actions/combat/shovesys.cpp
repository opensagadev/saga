...
void ShoveSystemCheckGameObject(GameObject_s *object) {
    if (object->field_0xefc & 1) return;
    f32 best_penetration = 0.0f;
    ShoveObject *shove = shovesys.objects;
    for (i32 i = 0; i < shovesys.count; i++, shove++) {
        if (shove->character_id != -1 &&
            shove->character_id == object->apiobj.supporting_platform_id &&
            object->contact_normal.y > 0.707f) continue;
        f32 reach = object->apiobj.field_0x1dc + shove->radius + 0.1f;
        if (!(reach > fabsf(object->apiobj.collision_position.x - shove->position.x))) continue;
        if (!(reach > fabsf(object->apiobj.collision_position.z - shove->position.z))) continue;
        if (!(shove->position.y + shove->radius > object->apiobj.collision_min.y)) continue;
        ...
        if (!BoundingBoxToLine(&minimum, &maximum, matrix, &object->apiobj.collision_min,
                               &object->apiobj.collision_max, object->apiobj.field_0x1dc + 0.1f, &point)) continue;
