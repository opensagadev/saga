...
void AITriggerSetSysProcess(AITRIGGERSETSYS_s *system) {
    ...
    for (i32 set_index = 0; set_index < 32; ++set_index, ++set) {
        ...
        for (i32 index = 0; index < set->trigger_count; ++index) {
            ...
            if (gizmo == NULL) {
                ...
                if (GameObjectUsingLever(player, lever) ||
                    (player2 != NULL && GameObjectUsingLever(player2, lever))) {
                    set->field_0x20e |= bit;
                ...
                if (obstacle->triggering_object == player ||
                    ...
                    if (obstacle->anim_set != NULL && (obstacle->anim_set->state == 3 ||
                        obstacle->anim_set->state == 0 || obstacle->anim_set->state == 4))
                        set->field_0x20e &= ~bit;
                ...
                GIZSPINNER_s *spinner = static_cast<GIZSPINNER_s *>(gizmo->object);
                if ((spinner->flags & 6) || ((spinner->flags & 8) && !ShadowMode) ||
                    spinner->field_70 == 1.0f || (spinner->state_flags & 0x20)) {
                    set->field_0x20e &= ~bit;
                ...
                if (((i8)player->character_context == LEGOCONTEXT_GRAPPLE && player->field_0x788 == grapple) ||
                    (player2 != NULL && (i8)player2->character_context == LEGOCONTEXT_GRAPPLE && player2->field_0x788 == grapple)) {
                    set->field_0x20e |= bit;
                } else if (set->field_0x20e & bit) {
                    set->field_0x20e &= ~bit;
        ...
        if ((set->field_0x20e != 0 || (set->flags & 2)) && !complete) {
            ...
            for (i32 index = 0; index < set->trigger_count && assigned_count + player_count < set->trigger_count; ++index) {
                if (!(set->field_0x20e & (1u << index)) && set->trigger_indices[index] == -1) {
                    ...
                    if (gizmo->type_id == lever_gizmotype_id) {
                        ...
                        GIZFORCE_s *force = static_cast<GIZFORCE_s *>(gizmo->object);
                        available = (force->progress_flags & 1) && force->using_object == NULL && force->field_0x3c == 0;
                    } else if (gizmo->type_id == grapple_gizmotype_id) {
                            ...
                            if (!FreePlay) {
                                ...
                                if (target->type_id == force_gizmotype_id) {
                                    ...
                                    if (force->config_flags & 0x10) {
                                        if (!CharCategory_IsCategory(object, 1)) continue;
                                        if (!(force->config_flags & 0x10) && !CharCategory_IsCategory(object, 0)) continue;
                                    } else if (!CharCategory_IsCategory(object, 0)) {
                                        continue;
                                    }
                                } else if (target->type_id == obstacle_gizmotype_id) {
                                    if (static_cast<GIZOBSTACLE_s *>(target->object)->mode == 7) continue;
