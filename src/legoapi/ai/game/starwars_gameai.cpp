...
static i32 StarWars_PrepareTakeOverJump(AIPACKET_s *packet, APIOBJECT_s *object, i32) {
    ...
    if ((owner->apiobj.character_data->game_character->flags_090 & 0x40) != 0) {
        ...
        if (participant != NULL) {
            AIPATHNODE *destination = &packet->path_info.path->nodes[
...
static i32 StarWars_PrepareHoverTube(AIPACKET_s *packet, APIOBJECT_s *object, i32) {
    GameObject_s *owner = object->objptr;
    AIPATH *path = packet->path_info.path;
    AIPATHCNX *connection = packet->path_info.connection;
    i32 destination_index = connection->node_indices[packet->path_info.direction == 0];
    AIPATHNODE *destination = &path->nodes[destination_index];
    AIPATHNODE *source = &path->nodes[connection->node_indices[packet->path_info.direction]];
    ...
    if ((packet->path_info.connection->traversal_flags[packet->path_info.direction] & 0x200) == 0 &&
        ((path->previous_inside_node_bits[destination_index >> 3] >> (destination_index & 7)) & 1) != 0) {
        if (TryToTeleportToNextNode(owner, destination, 0) != 0) {
            return 1;
        }
        packet->path_connection_state = 0;
    } else {
        if (packet->path_connection_state != 0 && TryToTeleportToNextNode(owner, destination, 0) != 0) {
            return 1;
        }
        switch (packet->path_connection_state) {
        case 0:
            if (ObjInTube(owner) == 0) {
                break;
            }
            packet->path_connection_state = 1;
            // The original immediately checks the newly entered tube state.
        case 1:
            if (ObjInTube(owner) != 0 &&
                !(owner->apiobj.collision_min.y > static_cast<TUBE *>(owner->field_0x788)->top &&
                  owner->apiobj.velocity.y > 0.0f)) {
                break;
            }
        case 2:
            packet->path_connection_state = 0;
            SetSpecialMove(owner, destination, NULL, 0);
...
STARWARS_PREPARE_SPECIAL_MOVE starwars_prepareforspecialmove[] = {
    {0x23, StarWars_PrepareJump},
    {4, StarWars_PrepareR2D2Glide},
    {0x18, StarWars_PrepareHatch},
    {0x40, StarWars_PrepareHoverTube},
    {0x1000, StarWars_PrepareBigJump},
    {0x8000, StarWars_PrepareTakeOverJump},
    {0x20000000, StarWars_PrepareObstacle},
    {0, NULL},
};
...
static i32 StarWars_PreparingForSpecialMove(AIPACKET_s *packet, APIOBJECT_s *object, i32 checks) {
    ...
    if (flags != 0 && (flags & 0x40000000) == 0) {
        ...
        if ((flags & 0x8000) != 0 &&
            (owner->apiobj.character_data->game_character->flags_090 & 0x40) == 0 &&
            owner->takeover_target == NULL && owner->character_context != 0x3c &&
            (capabilities & 0xffff7fff) == 0) {
    ...
    if ((packet->movement_event_flags & 0x20) != 0 &&
        (owner->tag_context_flags & 2) == 0 && drop_back_in_timer > 0.0f &&
        (packet->path_info.connection->traversal_flags[packet->path_info.direction] & 0xd8000000) == 0) {
        TryToTeleportToNextNode(owner, &packet->path_info.path->nodes[
            packet->path_info.connection->node_indices[packet->path_info.direction == 0]], 0);
    }
    return 0;
}
...
static i32 StarWars_MidSpecialMove_Default(AISYS_s *system, AIPACKET_s *packet, APIOBJECT_s *object) {
    GameObject_s *owner = object->objptr;
    u32 capabilities = packet->path_info.connection->traversal_flags[packet->path_info.direction] & packet->capabilities;
    AIPATHNODE *nodes = packet->path_info.path->nodes;
    ...
    if (node->has_special != 0 && (node->runtime_flags & 2) != 0) {
        ...
        if (owner->apiobj.field_0x27d != 0 && owner->apiobj.supporting_platform_id != -1 && CurTerr != NULL) {
            ...
            check_inside = instance == static_cast<i16>(CurTerr->platforms[owner->apiobj.supporting_platform_id].scene_object_index);
        }
...
i32 StarWars_PrepareJump(AIPACKET_s *packet, APIOBJECT_s *object, i32 checks) {
    ...
    if ((connection->traversal_flags[packet->path_info.direction] & 0x200) == 0 &&
        ...
        ((packet->path_info.path->previous_inside_node_bits[destination_index >> 3] >> (destination_index & 7)) & 1) != 0) {
        if (TryToTeleportToNextNode(owner, destination, 0) != 0) {
            return 1;
        }
        packet->path_connection_state = 1;
    ...
    f32 distance = NuVecXZDistSqr(&object->position, &packet->movement_destination, &offset);
    i32 ready = 0;
    if (checks != 0 && (ai_moveradius * ai_moveradius > distance ||
        owner->apiobj.respawn_timer > jump_stuck_time || owner->character_context == 0x44)) {
        ready = 1;
        packet->movement_destination = object->position;
    }
    packet->movement_stopping_distance = 0.0f;
    f32 jump_distance = ((flags & capabilities) & 0x20) != 0 ? 1.5f :
                        (((flags & capabilities) & 2) != 0 ? 1.2f : 0.8f);
    connection = packet->path_info.connection;
        ...
        if ((connection->traversal_flags[packet->path_info.direction] & 0x200) == 0) {
            ...
            for (i32 index = 0; index < object_count; ++index, ++other) {
                ...
                if (other != current_player && other->ai.path_info.connection == connection &&
                    other->ai.special_move_node == NULL && other->ai.path_connection_state < packet->path_connection_state) {
                    continue;
                }
