...
GIZMOBLOWUP_s *GizmoBlowUp_Target(GameObject_s *object, nuvec_s *position, nuvec_s *direction,
    ...
    if (blowup == NULL) return NULL;
    if (GizmoBlowUp_NoTargetFn != NULL && GizmoBlowUp_NoTargetFn(world, object) != 0) return NULL;
    NUVEC aim = *direction;
    ...
    if (count <= 0) {
        ...
        if (world->field_0x50d0 == 0) blowup = world->gizmo_blowups;
    }
    GIZMOBLOWUP_s *nearest = NULL, *fallback = NULL;
    f32 nearest_distance = 100000000.0f;
    for (i32 i = 0; i < count; ++i, ++blowup) {
        if (world->field_0x50d0 != 0) blowup = world->blowup_target_candidates[i];
        if ((blowup->draw_flags & 0x8000) == 0 || (blowup->output_flags & 1) != 0 ||
            (blowup->status_flags & 0x08800000) == 0 || (blowup->visibility_flags & 0x40) == 0) continue;
        if ((blowup->draw_flags & 0x20) != 0 && ShadowMode == 0) continue;
        if (blowup->mid_position.x < min_x || blowup->mid_position.x > max_x ||
            blowup->mid_position.z < min_z || blowup->mid_position.z > max_z) continue;
        NUVEC delta;
        ...
        u16 angle = distance < near_distance && directional != 0 ? near_angle :
                    distance < mid_distance ? mid_angle : far_angle;
        if (!(dot > NuTrigTable[((i32)angle + 0x4000) >> 1 & 0x7fff]) || !(nearest_distance > distance)) continue;
...
GIZMOBLOWUP_s *GizmoBlowUp_FindByName(WORLDINFO_s *world, char *name) {
    ...
    if (blowup != NULL) {
        for (i32 i = 0; i < world->gizmo_blowup_count; ++i, ++blowup) {
            if (NuStrICmp(blowup->name, name) == 0) return blowup;
        }
    }
    return NULL;
}
    ...
    if (blowup != NULL) {
        for (i32 i = 0; i < world->gizmo_blowup_count; ++i, ++blowup) {
            if (blowup->platform_id == platform_id) {
                if ((blowup->status_flags & 0x804001) == 0x804000) return blowup;
                if (fallback == NULL) fallback = blowup;
            }
        }
    }
    return fallback;
