...
EXPLOSION *AddExplosion(nuvec_s *position, float radius, float strength, GameObject_s *object,
                        i32 effect, i32 flags) {
...
void UpdateExplosion_Generic(EXPLOSION *explosion) {
    ...
    if ((explosion->field_0x24 & 0x100) == 0) {
        for (i32 i = 0; i < HIGHGAMEOBJECT; ++i) {
            ...
            if ((object->field_0x1f8 & 0x1001) != 0x1001 || object->field_0x287 != 0) continue;
            if ((CInfo[static_cast<i8>(target->character_context)].flags & 0x20008000) != 0) continue;
            if ((static_cast<GAMECHARACTERDATA_s *>(object->character_data->field11_0x24)->flags_090 & 0x8000) != 0) continue;
            if ((explosion->field_0x24 & 0x80) != 0 && (object->character_data->model_flags & 0x10) != 0) continue;
            if ((explosion->field_0x24 & 0x4000) != 0 && object->field_0x27c != -1) continue;
            if ((explosion->field_0x24 & 8) == 0 &&
                (((object->field_0x1e4 & explosion->field_0x00) | (object->field_0x1e8 & explosion->field_0x04)) != 0 ||
                 explosion->object == target)) continue;
            if (object->collision_min.x > maximum.x || minimum.x > object->collision_max.x ||
                ...
                object->collision_min.z > maximum.z || minimum.z > object->collision_max.z) continue;
            if (!SphereSphereOverlapScaleY(&object->collision_position, object->field_0x1dc, object->field_0x1e0,
                                          &explosion->position, radius, radius)) continue;
            if ((explosion->field_0x24 & 8) != 0) {
            ...
            if (hit_character || (explosion->field_0x24 & 1) == 0) continue;
            if (((object->field_0x1e4 & explosion->field_0x00) | (object->field_0x1e8 & explosion->field_0x04)) != 0 ||
                explosion->object == target || target->spawn_protection_timer > 0.0f || (target->field_0xefe & 0x40) != 0) continue;
            if (object->field_0x27c != -1 && target->field_0x1024 > 0.0f && (explosion->field_0x24 & 0x2000) == 0) continue;
            ...
            if (source != NULL && !(Arcade_GetMode(NULL) == 99 && (explosion->field_0x24 & 0x10010) != 0)) {
                ...
                if (target_player == source_player && (!target_player || target->field_0xd24 != 1.0f) &&
                    ((object->field_0x1f4 ^ source->apiobj.field_0x1f4) & 1) == 0) continue;
            }
            ...
            if (target->field_0xd24 >= 0.99f) {
                ...
                if ((explosion->field_0x24 & 0x20) == 0) { hit_character = true; continue; }
            }
            if (CannotKill(target)) { hit_character = true; continue; }
            i32 damage;
            if ((object->flags_low & 0x80) != 0 && Player_HasInvincibility(target)) damage = 0;
            else if ((explosion->field_0x24 & 0x800) != 0 && (target->field_0xefb & 8) == 0) damage = -1;
            else if ((explosion->field_0x24 & 0x20) != 0 && (target->field_0xefb & 8) == 0 && static_cast<i8>(target->current_hp) > 0) damage = -1;
            else damage = explosion->field_0x32;
            source = explosion->object;
            if (source == NULL) {
                if (explosion->field_0x33 == 0) source = Player[0];
                else if (explosion->field_0x33 == 1) source = Player[1];
            }
            ...
            if (ObjHitObj(source, target, damage, hit_flags, 0, 1) == 2) {
                if (source != NULL) NewRumble(source->pad_gamepad->pad, 0.5f, 0);
                ++explosion->field_0x30;
                if ((explosion->field_0x24 & 0x200) != 0 && explosion->field_0x33 <= 1 && Arcade != 0) {
                    if (static_cast<u8>(object->field_0x27c) <= 1) Arcade_PlayerKilled((object->field_0x27c ^ 1) & 1, 0);
                    else if (object->field_0x27c == -1) Arcade_AIKilled(static_cast<i8>(explosion->field_0x33));
                }
            }
            if ((explosion->field_0x24 & 0x40) != 0) {
                NewRumbleAllPlayers(0.5f, 0.1f, 0, 0);
                GameCam_HitJudder();
            } else if (explosion->object != NULL) NewBuzz(explosion->object->pad_gamepad->pad, 0.1f, 0);
            hit_character = true;
        }
    }
    if (!hit_character && (explosion->field_0x24 & 2) != 0) {
        u32 hit_type;
        if ((explosion->field_0x24 & 0x10) != 0) hit_type = 3;
        else if ((explosion->field_0x24 & 0x100) != 0) hit_type = 7;
        else if ((explosion->field_0x24 & 0x200) != 0) hit_type = 8;
        else hit_type = (explosion->field_0x24 & 0x8000) != 0 ? 9 : 2;
        if (GizmoBlowUp_Hit(explosion->object, &explosion->position, 1, radius, &minimum, &maximum, NULL, hit_type, NULL)) {
            if ((explosion->field_0x24 & 0x40) != 0) {
                NewRumbleAllPlayers(0.5f, 0.1f, 0, 0);
                GameCam_HitJudder();
            } else if (explosion->object != NULL) NewBuzz(explosion->object->pad_gamepad->pad, 0.1f, 0);
