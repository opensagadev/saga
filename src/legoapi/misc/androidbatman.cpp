...
void NewScanRot(nuvec_s *position, i32 terrain_mask) {
    ...
    for (i32 i = 0; i < 16; ++i) {
        ...
        if (cache.cache_age < CurTerr->index_levels[cache_index].cache_age) cache_index = i;
    }
        ...
        for (;;) {
            ...
            for (i32 i = 0; i < count; ++i) {
                ...
                if (local_max_x < shape->min_x || shape->max_x <= local_min_x ||
                    local_max_z < shape->min_z || shape->max_z <= local_min_z) continue;
                if (shape->material[1] != 0 && (shape->material[1] & terrain_mask) == 0) continue;
                if (reinterpret_cast<u8 *>(writer.cursor) >= writer.limit) continue;
                *writer.cursor++ = shape;
                ++writer.shape_count;
            }
            ShadowFinishGroup(&writer, group_index);
    ...
    if (NuTerrPlatsOff == 0) {
        ...
        for (i32 p = 0; p < platform_count; ++p) {
            ...
            if (group.origin.x - group.radius > max_x || min_x > group.origin.x + group.radius ||
                group.origin.z - group.radius > max_z || min_z > group.origin.z + group.radius) continue;
            ...
            if (rotating) {
                ...
                if (!(radius * radius > dx * dx + dz * dz)) continue;
            } else if (!ShadowBoundsOverlap(local_min_x, local_min_z, local_max_x, local_max_z,
                                             group.bounds_min, group.bounds_max) || group.chunk_type == -1) {
                continue;
            }
            if (platform.scene_transform != NULL &&
                (*static_cast<u8 *>(platform.scene_transform) & ((platform.flags & 4) != 0 ? 2 : 1)) == 0) continue;
            TERRAIN_SHAPE_BATCH *batch = static_cast<TERRAIN_SHAPE_BATCH *>(group.data);
            while (batch->marker >= 0) {
                TERRAIN_SHAPE *shapes = reinterpret_cast<TERRAIN_SHAPE *>(batch + 1);
                ...
                for (i32 i = 0; batch_overlaps && i < batch->shape_count; ++i) {
                    ...
                    if (!rotating) {
                        ...
                        if (!above_x || !below_x || !above_z || !below_z) continue;
                        ...
                        *reinterpret_cast<u32 *>(transformed->material) = *reinterpret_cast<u32 *>(shape->material);
                        ...
                        for (i32 n = quad ? 1 : 0; n >= 0; --n) {
                            ...
                            NUVEC a = {vertices[first].x - vertices[origin].x,
                                       vertices[first].y - vertices[origin].y,
                                       vertices[first].z - vertices[origin].z};
                            NUVEC b = {vertices[second].x - vertices[origin].x,
                                       vertices[second].y - vertices[origin].y,
