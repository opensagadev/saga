...
static i32 LedgeTerrain_Attach(GameObject_s *object, u16 facing, NUVEC *position, u16 *wall_angle) {
    *wall_angle = NuAtan2D(object->contact_normal.x, object->contact_normal.z);
    ...
    if (height == 2000000.0f) return 0;
    if (NuTrigTable[0x2555] > ShadNorm.y) return 0;
    f32 height_difference = position->y - height;
    if (height_difference > 0.025f || height_difference < -0.1f) return 0;
    if (object->apiobj.field_0x220 != 2000000.0f && height > object->apiobj.field_0x220) return 0;
    position->y = height;
    return 1;
}
...
LEDGE *Ledge_FindNearest(WORLDINFO_s *world, NUVEC *position, GameObject_s *object, f32 *distance_squared) {
    ...
    for (i32 i = 0; i < world->ledge_count; ++i, ++ledge) {
        if (object != NULL) {
            if ((ledge->state_flags & 3) != 3) continue;
            if ((ledge->flags & 1) && ShadowMode == 0) continue;
        }
...
void LedgeTerrain_MoveCode(GameObject_s *object) {
    if (LEGOCONTEXT_LEDGETERRAIN == -1) return;
    NUVEC position;
    u16 wall_angle;
    if (object->character_context != LEGOCONTEXT_LEDGETERRAIN) {
        if (object->apiobj.field_0x27d != 0 || !(object->apiobj.velocity.y <= 0.0f)) return;
        if (LedgeTerrain_CheckAnims && (LEGOACT_LEDGE_IDLE == -1 ||
            object->apiobj.character_model->model_data_b[LEGOACT_LEDGE_IDLE] == NULL)) return;
        if (object->character_context != -1 &&
            ...
            if (LEGOCONTEXT_JUMP == -1 || object->character_context != LEGOCONTEXT_JUMP ||
                !(object->context_animation_timer >= 0.1f)) return;
        }
        if (!(object->apiobj.field_0x1f8 & 0x80) && !(object->field_0xf01 & 0x80)) return;
        if (!(object->pad_gamepad->input_magnitude > 0.0f)) return;
        u16 facing = GamePad_InputAngle(object, object->pad_gamepad);
        if (!object->field_0x1084) return;
        u8 surface = object->field_0x6b0;
        if (surface < 32 && (TerSurface[surface].flags & 0x581)) return;
        if (object->apiobj.collision_position.y - 0.05f >= object->contact_position.y ||
            fabsf(object->contact_normal.y) > NuTrigTable[0x3000]) return;
        if (!LedgeTerrain_Attach(object, facing, &position, &wall_angle)) return;
    ...
    if (object->pad_gamepad->buttons_pressed & GAMEPAD_JUMP) {
        ...
        if (height > 0.0f) object->apiobj.velocity.y = NuFsqrt(-2.0f * object->apiobj.character_data->game_character->gravity * height);
        return;
    }
    ...
    if (!object->field_0x7a3 && object->context_animation_timer >= object->field_0x768) object->field_0x7a3 = 1;
    f32 travelled = 0.0f;
    u8 surface = object->field_0x6b0;
    bool attached = object->field_0x1084 &&
        !(surface < 32 && (TerSurface[surface].flags & 0x581)) &&
    ...
    if (action != -1 && object->apiobj.character_model->model_data_b[action] != NULL) {
        ...
        if (LedgeTerrain_CheckAnims && !AnimPlaying(&object->apiobj.anim_packet, object->context_animation, 1, 0)) return;
    } else if (LedgeTerrain_CheckAnims) {
        return;
    }
    if (speed < 0.0f) travelled = -travelled;
...
i32 LedgeTerrain_SetTargetMom(GameObject_s *object) {
    object->target_velocity.x = (NU_SIN_LUT(object->apiobj.movement_facing_angle) * LEDGETERRAINLOOKAHEAD +
        object->external_force.x - object->apiobj.upper_position.x) * 10.0f;
    object->target_velocity.z = (NU_COS_LUT(object->apiobj.movement_facing_angle) * LEDGETERRAINLOOKAHEAD +
        object->external_force.z - object->apiobj.upper_position.z) * 10.0f;
